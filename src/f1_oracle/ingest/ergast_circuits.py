"""
Ergast circuits ingestion (raw layer).
...
This module mirrors the style of ergast_calendar.py, but targets the circuits endpoint.
"""
...
def ingest_circuits_for_season(
    season: int,
    raw_dir: str | Path = "data/raw",
    client: ErgastClient | None = None,
) -> Path:
    """
    Fetch and store the Ergast circuits snapshot for a season.
    ...
    raw_text = c.get_text(
        f"{season}/circuits.json",
        params={"limit": 1000},
    )

    out_path = out_dir / "circuits.raw.json"
