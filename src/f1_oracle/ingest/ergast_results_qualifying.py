"""
Ergast qualifying results ingestion (raw layer).
...
Naming (locked):
- CLI: f1-oracle ingest ergast results qualifying --season YYYY
- Raw snapshot: data/raw/ergast/YYYY/results_qualifying.raw.json
...
def ingest_results_qualifying_for_season(
    season: int,
    raw_dir: str | Path = "data/raw",
    client: ErgastClient | None = None,
) -> Path:
    """
    Fetch and store the Ergast qualifying results snapshot for a season.
    ...
    # Write compact JSON. We are not prettifying/normalizing; we are only combining pages.
    out_path = out_dir / "results_qualifying.raw.json"
